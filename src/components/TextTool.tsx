...
import { Loader2, Copy, Check, Sparkles, FileText, Download, Send, Wand2 } from "lucide-react";
...
export function TextTool({ id, name, description, placeholder, systemInstruction, promptPrefix, icon: Icon, color }: TextToolProps) {
          ...
          <div className="glass-card p-5 md:p-8 rounded-[2rem] md:rounded-[3rem] shadow-xl border-white/40 space-y-6 md:space-y-8">
            ...
            <div className="space-y-4">
              <div className={cn(
                ...
                <textarea
                  className="w-full p-5 md:p-8 rounded-2xl md:rounded-[2rem] bg-transparent outline-none min-h-[250px] md:min-h-[300px] text-sm md:text-base text-text-main leading-relaxed font-medium placeholder:text-text-muted resize-none"
              ...
              {result && (
                <div className="flex items-center gap-2 md:gap-3">
                  <Button 
                    variant="outline" 
                    size="sm" 
                    className="flex-1 sm:flex-none rounded-xl md:rounded-2xl text-text-muted hover:bg-bg-main font-black px-4 md:px-6 h-10 md:h-12 border-2 text-[9px] md:text-[10px] uppercase tracking-widest"
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className="flex-1 sm:flex-none rounded-xl md:rounded-2xl text-indigo-500 hover:bg-indigo-500/10 font-black px-4 md:px-6 h-10 md:h-12 border border-indigo-500/20 text-[9px] md:text-[10px] uppercase tracking-widest"
            ...
            <div className="flex-grow overflow-x-hidden">
              {loading ? (
                <div className="space-y-8 md:space-y-10 animate-pulse p-2 md:p-4">
                  ...
                  <div className="space-y-4">
                    <div className="h-5 w-full bg-border-main rounded-full" />
                    <div className="h-5 w-5/6 bg-border-main rounded-full" />
                  </div>
                </div>
              ) : result ? (
                <motion.div 
